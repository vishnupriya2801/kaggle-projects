...
hyper_params <- list(activation = activation_opt,
                     hidden=hidden)
grid_m <- h2o.grid("deeplearning", x = 1:6, y = 'left',
                   grid_id = "grid",
...
gridperf_n <- h2o.getGrid(grid_id = "grid", 
                          sort_by = "auc", 
                          decreasing = TRUE)
print(gridperf_n)
...
model = h2o.deeplearning(x=1:6,y = 'left',
                         training_frame = as.h2o(training_set),
                         validation_frame = as.h2o(validation_set),
                         nfolds =10,
...
y_pred = h2o.predict(model, newdata = as.h2o(test_set[-7]))
y_pred.R=as.data.frame(y_pred)
test_set.R=as.data.frame(test_set)
head(y_pred.R)

cm_d=confusionMatrix(y_pred.R[,1],test_set.R[,7])
cm_d
...
for(i in 1:6)
{
  model1 = h2o.deeplearning(x=1:6,y = 'left',
                            training_frame = as.h2o(training_set),
                            validation_frame = as.h2o(validation_set),
                            nfolds =10,
                            keep_cross_validation_predictions=TRUE,
