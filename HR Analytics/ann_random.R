...
#########
Out <- RPParallel(XTrain =training_set[,-10], YTrain =training_set[,10], XTest =test_set[,-10], d = 2, 
...
splits <- h2o.splitFrame(data = as.h2o(Out), 
...
hyper_params <- list(activation = activation_opt,
                     hidden=hidden)
grid_m <- h2o.grid("deeplearning", x = 1:10, y = 'left',
                   grid_id = "grid_n19",
...
gridperf_n <- h2o.getGrid(grid_id = "grid_n19", 
                          sort_by = "auc", 
                          decreasing = TRUE)
print(gridperf_n)
...
model = h2o.deeplearning(x=1:10,y = 'left',
                         training_frame = as.h2o(training_set),
                         validation_frame = as.h2o(validation_set),
                         nfolds =10,
...
y_pred.R[,1]=y_pred.R[,1] - 0.5
y_pred.R = (y_pred.R >= 0.5)
y_pred=as.matrix(y_pred)
y_pred =y_pred +0

cm_d=confusionMatrix(y_pred[,1],test_set.R[,11])
cm_d
