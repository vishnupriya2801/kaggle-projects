...
#sample=data[sample(nrow(data),70),]
#?sample
...
set.seed(29)
kmeans = kmeans(x = data, centers =3,nstart=25 )
fviz_cluster(kmeans, data = data)

#MODIFYING DATA
clust=kmeans$cluster
clust=as.data.frame(clust)
new=mat.or.vec(14999,3)
left=dataset[,10]
new=cbind(new,left)
for (i in 1:14999){
  x=clust[i,1]
...
hyper_params <- list(activation = activation_opt,
                     hidden=hidden)
grid_m <- h2o.grid("deeplearning", x = 1:3, y = 'left',
                   grid_id = "grid_3a",
...
gridperf_n <- h2o.getGrid(grid_id = "grid_3a", 
                          sort_by = "auc", 
                          decreasing = TRUE)
print(gridperf_n)
...
model = h2o.deeplearning(x=1:3,y = 'left',
                         training_frame = as.h2o(training_set),
                         validation_frame = as.h2o(validation_set),
                         nfolds =10,
...
for(i in 1:6)
{
  model1 = h2o.deeplearning(x=1:3,y = 'left',
                            training_frame = as.h2o(training_set),
                            validation_frame = as.h2o(validation_set),
                            nfolds =10,
                            keep_cross_validation_predictions=TRUE,
