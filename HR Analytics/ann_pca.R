...
preprocessParams <- preProcess(dataset[,-10], method=c("center", "scale", "pca"))
# summarize transform parameters
print(preprocessParams)
# transform the dataset using the parameters
transformed <- predict(preprocessParams, dataset[,-10])
left=dataset[,10]
transformed=cbind.data.frame(transformed,left)
#transformed=transformed[,-11]
...
hyper_params <- list(activation = activation_opt,
                     hidden=hidden)
grid_m <- h2o.grid("deeplearning", x = 1:9, y = 'left',
                   grid_id = "grid_9",
...
gridperf_n <- h2o.getGrid(grid_id = "grid_9", 
                          sort_by = "auc", 
                          decreasing = TRUE)
print(gridperf_n)
...
model = h2o.deeplearning(x=1:9,y = 'left',
                         training_frame = as.h2o(training_set),
                         validation_frame = as.h2o(validation_set),
                         nfolds =10,
...
cm_d=confusionMatrix(y_pred.R[,1],test_set.R[,10])
cm_d
...
for(i in 1:6)
{
  model1 = h2o.deeplearning(x=1:9,y = 'left',
                            training_frame = as.h2o(training_set),
                            validation_frame = as.h2o(validation_set),
                            nfolds =10,
                            keep_cross_validation_predictions=TRUE,
